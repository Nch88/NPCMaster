...
//	dc.createGenerationVectors(dictionary, generationVectors, c);
//
//	vector<unsigned long> terminals = { 's', 'i', 'n', 'g', '.', 'd', 'o', 'w', 'a', 'h', 'y', 'u', 'm' };
...
//	dense_hash_map<unsigned long, unsigned long> tIndices;
...
//	dictionary[307] = H;
//
//	unordered_set<unsigned long> terminals = { 's', 'i', 'n', 'g', '.', 'd', 'o', 'w', 'a', 'h', 'y', 'u', 'm' };
//
...
//	dc.generateCompactDictionary(dictionary, terminals, terminalVector, pairs, indices, c);
//
//	vector<vector<CompactPair>> expected;
//	vector<CompactPair> v1, v2, v3;
//	expected.push_back(v1);
...
//		for (int j = 0; j < pairs[i].size(); ++j)
//		{
//			ASSERT_EQ(expected[i][j].leftSymbol, dictionary[pairs[i][j]].leftSymbol);
//			ASSERT_EQ(expected[i][j].rightSymbol, dictionary[pairs[i][j]].rightSymbol);
//		}
//	}
//
//	//Check indices
